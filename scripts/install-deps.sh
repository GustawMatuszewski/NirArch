...
if [[ "$DISTRO" == "arch" ]]; then
    $INSTALL_CMD niri wofi alacritty waybar swaync pipewire pipewire-pulse pipewire-alsa wireplumber networkmanager bluez bluez-utils swww libnotify btop pavucontrol pulsemixer bluetuith-bin swaylock-effects playerctl ttf-nerd-fonts-symbols-common ttf-jetbrains-mono-nerd noto-fonts-emoji
elif [[ "$DISTRO" == "fedora" ]]; then
    $INSTALL_CMD niri wofi alacritty waybar swaync pipewire-pulseaudio pipewire-utils wireplumber NetworkManager bluez bluez-utils swww libnotify btop pavucontrol pulsemixer swaylock-effects playerctl google-noto-emoji-fonts jetbrains-mono-fonts-all nerd-fonts flatpak
